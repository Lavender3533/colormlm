...
impl VulkanContext {
    pub fn init() -> Result<Self> {
        unsafe {
            ...
            let qf_graphics = find_queue_family(&instance, physical,
                vk::QueueFlags::GRAPHICS, vk::QueueFlags::empty())
                .ok_or_else(|| anyhow!("no graphics queue"))?;
            let qf_transfer = find_queue_family(&instance, physical,
            ...
            let mut qis = vec![
                vk::DeviceQueueCreateInfo::default()
                    .queue_family_index(qf_graphics).queue_priorities(&prio),
            ];
            if qf_transfer != qf_graphics {
                qis.push(vk::DeviceQueueCreateInfo::default()
                    .queue_family_index(qf_transfer).queue_priorities(&prio));
            }
            let device = instance.create_device(physical,
                &vk::DeviceCreateInfo::default().queue_create_infos(&qis), None)?;
            ...
            Ok(Self {
                entry, instance, physical, device,
                qf_graphics, qf_transfer, q_graphics, q_transfer,
                mem_props, gpu_name,
            })
        }
    }
...
unsafe fn find_queue_family(
    ...
    required: vk::QueueFlags, forbidden: vk::QueueFlags,
) -> Option<u32> {
