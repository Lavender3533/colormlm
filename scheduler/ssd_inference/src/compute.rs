...
pub const MATMUL_FP32_NAIVE_SPV: &[u8] = include_bytes!(concat!(env!("OUT_DIR"), "/matmul_fp32_naive.spv"));
...
pub const FUSED_Q4K_MATVEC_SPV: &[u8] = include_bytes!(concat!(env!("OUT_DIR"), "/fused_q4k_matvec.spv"));

pub const FUSED_Q5K_MATVEC_SPV: &[u8] = include_bytes!(concat!(env!("OUT_DIR"), "/fused_q5k_matvec.spv"));

pub const FUSED_Q6K_MATVEC_SPV: &[u8] = include_bytes!(concat!(env!("OUT_DIR"), "/fused_q6k_matvec.spv"));
...
pub const WEIGHTED_SUM_8_SPV: &[u8] = include_bytes!(concat!(env!("OUT_DIR"), "/weighted_sum_8.spv"));
    ...
    pub fn new(
        ...
        unsafe {
            ...
            let module = ctx.device.create_shader_module(
                &vk::ShaderModuleCreateInfo::default().code(&spv_words), None)?;
            ...
            Ok(Self {
                pipeline, layout: pipeline_layout, set_layout,
                shader_module: module, n_bindings: n_storage_buffers,
                push_bytes: push_constant_bytes,
            })
        }
    }

    pub fn n_bindings(&self) -> u32 { self.n_bindings }
    pub fn push_bytes(&self) -> u32 { self.push_bytes }
...
impl DescriptorArena {
    pub fn new(ctx: &VulkanContext, capacity: u32, max_bindings_per_set: u32) -> Result<Self> {
        ...
        let pool = unsafe { ctx.device.create_descriptor_pool(
            &vk::DescriptorPoolCreateInfo::default()
                .max_sets(capacity)
                .pool_sizes(&pool_sizes)
                .flags(vk::DescriptorPoolCreateFlags::FREE_DESCRIPTOR_SET),
            None)? };
        Ok(Self { pool, allocated: 0, capacity })
    }

    pub fn reset(&mut self, ctx: &VulkanContext) -> Result<()> {
        unsafe { ctx.device.reset_descriptor_pool(self.pool, vk::DescriptorPoolResetFlags::empty())?; }
        self.allocated = 0;
        Ok(())
    }

    pub fn alloc_set(&mut self, ctx: &VulkanContext, pipe: &ComputePipeline,
                     buffers: &[(&GpuBuffer, u64, u64)]) -> Result<vk::DescriptorSet> {
        assert!(self.allocated < self.capacity, "descriptor arena full ({}/{})", self.allocated, self.capacity);
        unsafe {
            let set_layouts = [pipe.set_layout];
            let set = ctx.device.allocate_descriptor_sets(
                &vk::DescriptorSetAllocateInfo::default()
                    .descriptor_pool(self.pool).set_layouts(&set_layouts))?[0];

            let infos: Vec<vk::DescriptorBufferInfo> = buffers.iter()
                .map(|(b, off, sz)| vk::DescriptorBufferInfo::default()
                    .buffer(b.handle()).offset(*off).range(*sz))
                .collect();
            let writes: Vec<vk::WriteDescriptorSet> = (0..pipe.n_bindings as usize)
                .map(|i| vk::WriteDescriptorSet::default()
                    .dst_set(set).dst_binding(i as u32).dst_array_element(0)
                    .descriptor_type(vk::DescriptorType::STORAGE_BUFFER)
...
impl DescriptorBinder {
    pub fn new(ctx: &VulkanContext, pipe: &ComputePipeline, buffers: &[(&GpuBuffer, u64)]) -> Result<Self> {
        let triples: Vec<(&GpuBuffer, u64, u64)> = buffers.iter().map(|(b, sz)| (*b, 0u64, *sz)).collect();
