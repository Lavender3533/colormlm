...
outpath = os.path.join(BASE, "fsq_generate_int8.py")

code = r'''# -*- coding: utf-8 -*-
"""FSQ Int8 Inference - Pre-scale integer matrix multiplication"""
import os, torch, json, time, gc
...
class Int8FSQModel:
    ...
    def _load_layer(self, idx):
        ...
        for key in self.weight_map:
            if key.startswith(prefix) and "experts." not in key and "._bq" not in key:
                w[key[len(prefix):]] = self._load(key).half()
        return w
    
    def _load_expert_raw(self, layer, expert, proj):
        """Load expert as uint8 codes + scale/offset (no dequantization)"""
    ...
    def _expert_matmul(self, x, codes, scale, offset):
        """Pre-scale integer matrix multiplication"""
        xs = x * scale.unsqueeze(0)
        off = (x * offset.unsqueeze(0)).sum(dim=-1, keepdim=True)
    ...
    def _rope(self, q, k, pos):
        ...
        h = q.shape[-1] // 2
        return (q * cos + torch.cat([-q[..., h:], q[..., :h]], dim=-1) * sin,
                k * cos + torch.cat([-k[..., h:], k[..., :h]], dim=-1) * sin)
    ...
    def forward(self, hidden, pos):
        for i in range(self.num_layers):
            ...
            q = (h @ w["self_attn.q_proj.weight"].T).view(B, S, self.num_heads, self.head_dim).transpose(1, 2)
            k = (h @ w["self_attn.k_proj.weight"].T).view(B, S, self.num_kv_heads, self.head_dim).transpose(1, 2)
            v = (h @ w["self_attn.v_proj.weight"].T).view(B, S, self.num_kv_heads, self.head_dim).transpose(1, 2)
            ...
            for j in range(self.top_k):
                ...
                gc, gs, go = self._load_expert_raw(i, idx, "gate_proj")
                uc, us, uo = self._load_expert_raw(i, idx, "up_proj")
                dc, ds, do_ = self._load_expert_raw(i, idx, "down_proj")
                
                gate = torch.nn.functional.silu(self._expert_matmul(h, gc, gs, go))
                up = self._expert_matmul(h, uc, us, uo)
                eo = self._expert_matmul(gate * up, dc, ds, do_)
                moe = moe + eo * wt
...
def main():
    ...
    print("=" * 50)
    print("FSQ Int8 Inference (pre-scale matmul)")
    print("=" * 50)
    m = Int8FSQModel(d)
