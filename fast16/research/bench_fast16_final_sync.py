"""Short A/B for omitting the already-synchronous CPU tail drain."""
...
OUTPUT = Path(__file__).resolve().parent / "fast16_final_sync_benchmark_report.json"
...
def main() -> int:
    ...
    skipped = run_arm("skip_cpu_final_sync", True)
    gain = (skipped["tokens_per_second"] / baseline["tokens_per_second"] - 1.0) * 100.0
    report = {
        "format": "fast16-final-sync-ab-v1",
        "model": MODEL.name,
        "decode_tokens": 64,
        "baseline": baseline,
        "skip_cpu_final_sync": skipped,
