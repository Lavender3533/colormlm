"""构建 40 层 Qwen3.6 router/expert/shared-expert 全局配对混合主干。"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="构建 v33 全局 Qwen3.6 MoE 配对")
    parser.add_argument(
        "--base",
        type=Path,
...
def main() -> int:
    ...
    if token_mismatches:
        raise RuntimeError(f"token ID 身份不一致: {token_mismatches}")
    ...
    if missing:
        raise RuntimeError(f"闭合 MoE 张量缺失: {missing[:4]}")
    ...
    copy_metadata(
        writer,
        base,
        {"general.name": "ColorLM-v33-Qwen36-Global-MoE-Pair"},
    )
    writer.add_string("colorlm.qwen36_global_moe.format", "closed-pair-v1")
    writer.add_string("colorlm.qwen36_global_moe.donor", args.donor.name)
    writer.add_uint32("colorlm.qwen36_global_moe.layer_count", LAYER_COUNT)
    writer.add_uint32("colorlm.qwen36_global_moe.tensor_count", len(replacements))
    ...
    report = {
        "format": "colormlm-qwen36-global-moe-pair-v1",
